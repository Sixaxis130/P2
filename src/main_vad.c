...
int main(int argc, char *argv[])
{
  ...
  last_state = ST_UNDEF;
  last_defined_state = ST_SILENCE; // Asumimos que venimos de silencio al inicializar
  ...
  for (t = last_t = 0;; t++){ /* For each frame ... */
    ...
    // Al especificar copiamos la trama del fichero original "wav" al fichero .wav output
    if (sndfile_out != 0){
    ...
    if (state != last_state)
    {
      if ((t != last_t) && (last_state == ST_UNDEF && vad_data->num_trama == 10)){ // Condición para el principio, solo ejecutamos una vez
  ...
  if (t != last_t){
    if (state == ST_UNDEF){
      fprintf(vadfile, "%.5f\t%.5f\t%s\n", last_t * frame_duration, t * frame_duration + n_read / (float)sf_info.samplerate, 
